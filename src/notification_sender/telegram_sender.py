...
class TelegramSender:
    ...
    def _should_fallback_to_plain_text(error_desc: str = "", response_text: str = "") -> bool:
        """Detect Telegram Markdown parsing failures or message-too-long that should retry as plain text."""
        ...
        markers = (
            ...
            "parse_mode",
            "markdown",
            "message is too long",   # FIX: handle oversized single chunk
        )
    ...
    def _split_section_hard(self, section: str, max_length: int) -> list:
        """FIX: Hard-cut a single section that is still larger than max_length."""
        lines = section.split("\n")
        ...
        if current:
            chunks.append("\n".join(current))
        return chunks if chunks else [section[:max_length]]

    def _send_telegram_chunked(self, api_url: str, chat_id: str, content: str, max_length: int,
                                message_thread_id: Optional[str] = None) -> bool:
        """分段发送长 Telegram 消息 (FIX: multi-separator + hard-cut fallback)"""

        # FIX: try splitting by section separator first, fallback to paragraph
        sections = content.split("\n---\n")
        if len(sections) == 1:
            sections = content.split("\n\n")

        # FIX: hard-cut any individual section that is still > max_length
        flat_sections = []
        for sec in sections:
            if len(sec) > max_length:
                flat_sections.extend(self._split_section_hard(sec, max_length))
            else:
        ...
        for section in flat_sections:
            ...
            if current_length + section_length > max_length:
                if current_chunk:
                    ...
                    logger.info(f"发送 Telegram 消息块 {chunk_index} ({len(chunk_content)} chars)...")
